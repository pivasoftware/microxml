/*
 * Index support code for microxml, a micro XML-like file parsing library.
 *
  ...
  */
...
#ifdef DEBUG
  {
	...
	printf("%8d  %-8p  %-14.14s  %s\n", i, ind->nodes[i],
	       ...
	       mxmlElementGetAttrValue(ind->nodes[i], attr));
    }
    else
    {
              ...
              mxml_node_t  *second)	/* I - Second node */
{
  ...
  */
  ...
  if (ind->attr)
  {
    if ((diff = strcmp(mxmlElementGetAttrValue(first, ind->attr),
                       mxmlElementGetAttrValue(second, ind->attr))) != 0)
      return (diff);
  }
           ...
           mxml_node_t  *node)		/* I - Node */
{
  ...
  */
  ...
  if (value)
  {
    if ((diff = strcmp(value, mxmlElementGetAttrValue(node, ind->attr))) != 0)
      return (diff);
  }
